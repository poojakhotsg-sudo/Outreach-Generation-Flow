...
async def get_suggested_solution(request: SuggestSolutionRequest):
    try:
        suggestion = await suggest_solution(request.selected_problems, request.research)
...
async def get_offer_guide(request: OfferRequest):
    try:
        guidance = await guide_offer(request.selected_problems, request.initial_solution, request.research)
